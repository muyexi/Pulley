...
open class PulleyViewController: UIViewController, PulleyDrawerViewControllerDelegate {
    ...
    public func setDidEndDecelerating(){
        ...
        let distanceFromBottomOfView = lowestStop + scrollView.contentOffset.y
        ...
        if scrollView == drawerScrollView
        {
            ...
            DispatchQueue.global().asyncAfter(deadline: .now() + 0.5) {
                DispatchQueue.main.async {[weak self]in
                    (self?.primaryContentViewController as? PulleyDrawerViewControllerDelegate)?.touchPassthroughScrollView?(scrollView: scrollView, point: CGPoint.zero)
                }
            }
